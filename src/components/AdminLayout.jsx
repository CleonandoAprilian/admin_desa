...
import Img1 from "../assets/sidoarum.png";
    ...
    <div className="flex min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 font-sans relative overflow-hidden">
      ...
      <aside className={`${sidebarOpen ? "w-72" : "w-20"} bg-slate-900/95 backdrop-blur-xl border-r border-white/10 text-white flex flex-col fixed h-full shadow-2xl transition-all duration-300 z-50`}>
        ...
        <div className="p-6 border-b border-white/10 flex items-center justify-between">
          {sidebarOpen && (
            <div className="flex items-center gap-3">
              <div>
                <img src={Img1} alt="Logo" className="text-white w-10 h-12" />
              </div>
              <div>
                <h1 className="text-lg font-bold tracking-wide">Admin Desa</h1>
                <p className="text-lg font-bold tracking-wide">Sidoarum</p>
              </div>
            </div>
          )}
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-4 px-4 py-3.5 rounded-xl transition-all duration-200 ${
                  isActive ? "bg-gradient-to-r from-blue-600 to-purple-600 text-white shadow-lg shadow-blue-500/30" : "text-slate-300 hover:bg-white/5 hover:text-white"
                }`}
        ...
        <div className="p-4 border-t border-white/10">
          <button onClick={handleLogout} className="flex items-center gap-4 w-full px-4 py-3.5 text-red-400 hover:bg-red-500/10 hover:text-red-300 rounded-xl transition-all" title={!sidebarOpen ? "Logout" : ""}>
